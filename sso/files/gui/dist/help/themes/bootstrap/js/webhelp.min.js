...
var Prism=function(){var l=/\blang(?:uage)?-(?!\*)(\w+)\b/i,d=self.Prism={util:{encode:function(a){return a instanceof n?new n(a.type,d.util.encode(a.content),a.alias):"Array"===d.util.type(a)?a.map(d.util.encode):a.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/\u00a0/g," ")},type:function(a){return Object.prototype.toString.call(a).match(/\[object (\w+)\]/)[1]},clone:function(a){switch(d.util.type(a)){case "Object":var e={},b;for(b in a)a.hasOwnProperty(b)&&(e[b]=d.util.clone(a[b]));return e;
    case "Array":return a.slice()}return a}},languages:{extend:function(a,e){var b=d.util.clone(d.languages[a]),c;for(c in e)b[c]=e[c];return b},insertBefore:function(a,e,b,c){c=c||d.languages;var f=c[a],h={},g;for(g in f)if(f.hasOwnProperty(g)){if(g==e)for(var k in b)b.hasOwnProperty(k)&&(h[k]=b[k]);h[g]=f[g]}return c[a]=h},DFS:function(a,e,b){for(var c in a)a.hasOwnProperty(c)&&(e.call(a,c,a[c],b||c),"Object"===d.util.type(a[c])?d.languages.DFS(a[c],e):"Array"===d.util.type(a[c])&&d.languages.DFS(a[c],
    e,c))}},highlightAll:function(a,e){for(var b=document.querySelectorAll('pre[class*="language-"], [class*="language-"] pre, pre[class*="lang-"], [class*="lang-"] pre'),c=0,f;f=b[c++];)d.highlightElement(f,!0===a,e)},highlightElement:function(a,e,b){for(var c,f,h=a;h&&!l.test(h.className);)h=h.parentNode;h&&(c=(h.className.match(l)||[,""])[1],f=d.languages[c]);if(f&&(a.className=a.className.replace(l,"").replace(/\s+/g," ")+" language-"+c,h=a.parentNode,/pre/i.test(h.nodeName)&&(h.className=h.className.replace(l,
        "").replace(/\s+/g," ")+" language-"+c),h=a.textContent)){var g={element:a,language:c,grammar:f,code:h};d.hooks.run("before-highlight",g);e&&self.Worker?(a=new Worker(d.filename),a.onmessage=function(a){g.highlightedCode=n.stringify(JSON.parse(a.data),c);d.hooks.run("before-insert",g);g.element.innerHTML=g.highlightedCode;b&&b.call(g.element);d.hooks.run("after-highlight",g)},a.postMessage(JSON.stringify({language:g.language,code:g.code}))):(g.highlightedCode=d.highlight(g.code,g.grammar,g.language),
    d.hooks.run("before-insert",g),g.element.innerHTML=g.highlightedCode,b&&b.call(a),d.hooks.run("after-highlight",g))}},highlight:function(a,e,b){a=d.tokenize(a,e);return n.stringify(d.util.encode(a),b)},tokenize:function(a,e,b){b=d.Token;var c=[a],f=e.rest;if(f){for(var h in f)e[h]=f[h];delete e.rest}a:for(h in e)if(e.hasOwnProperty(h)&&e[h])for(var f=e[h],f="Array"===d.util.type(f)?f:[f],g=0;g<f.length;++g)for(var k=f[g],l=k.inside,n=!!k.lookbehind,p=0,v=k.alias,k=k.pattern||k,u=0;u<c.length;u++){var q=
    c[u];if(c.length>a.length)break a;if(!(q instanceof b)){k.lastIndex=0;var m=k.exec(q);if(m){n&&(p=m[1].length);var t=m.index-1+p,m=m[0].slice(p),r=t+m.length,t=q.slice(0,t+1),q=q.slice(r+1),r=[u,1];t&&r.push(t);m=new b(h,l?d.tokenize(m,l):m,v);r.push(m);q&&r.push(q);Array.prototype.splice.apply(c,r)}}}return c},hooks:{all:{},add:function(a,e){var b=d.hooks.all;b[a]=b[a]||[];b[a].push(e)},run:function(a,e){var b=d.hooks.all[a];if(b&&b.length)for(var c=0,f;f=b[c++];)f(e)}}},n=d.Token=function(a,d,b){this.type=
    a;this.content=d;this.alias=b};n.stringify=function(a,e,b){if("string"==typeof a)return a;if("[object Array]"==Object.prototype.toString.call(a))return a.map(function(b){return n.stringify(b,e,a)}).join("");b={type:a.type,content:n.stringify(a.content,e,b),tag:"span",classes:["token",a.type],attributes:{},language:e,parent:b};"comment"==b.type&&(b.attributes.spellcheck="true");if(a.alias){var c="Array"===d.util.type(a.alias)?a.alias:[a.alias];Array.prototype.push.apply(b.classes,c)}d.hooks.run("wrap",
    b);var c="",f;for(f in b.attributes)c+=f+'="'+(b.attributes[f]||"")+'"';return"<"+b.tag+' class="'+b.classes.join(" ")+'" '+c+">"+b.content+"</"+b.tag+">"};if(!self.document){if(!self.addEventListener)return self.Prism;self.addEventListener("message",function(a){a=JSON.parse(a.data);self.postMessage(JSON.stringify(d.util.encode(d.tokenize(a.code,d.languages[a.language]))));self.close()},!1);return self.Prism}var p=document.getElementsByTagName("script");if(p=p[p.length-1])d.filename=p.src,document.addEventListener&&
!p.hasAttribute("data-manual")&&document.addEventListener("DOMContentLoaded",d.highlightAll);return self.Prism}();"undefined"!==typeof module&&module.exports&&(module.exports=Prism);
Prism.languages.markup={comment:/\x3c!--[\w\W]*?--\x3e/g,prolog:/<\?.+?\?>/,doctype:/<!DOCTYPE.+?>/,cdata:/<!\[CDATA\[[\w\W]*?]]\x3e/i,tag:{pattern:/<\/?[\w:-]+\s*(?:\s+[\w:-]+(?:=(?:("|')(\\?[\w\W])*?\1|[^\s'">=]+))?\s*)*\/?>/gi,inside:{tag:{pattern:/^<\/?[\w:-]+/i,inside:{punctuation:/^<\/?/,namespace:/^[\w-]+?:/}},"attr-value":{pattern:/=(?:('|")[\w\W]*?(\1)|[^\s>]+)/gi,inside:{punctuation:/=|>|"/g}},punctuation:/\/?>/g,"attr-name":{pattern:/[\w:-]+/g,inside:{namespace:/^[\w-]+?:/}}}},entity:/\&#?[\da-z]{1,8};/gi};
Prism.hooks.add("wrap",function(l){"entity"===l.type&&(l.attributes.title=l.content.replace(/&amp;/,"&"))});Prism.languages.css={comment:/\/\*[\w\W]*?\*\//g,atrule:{pattern:/@[\w-]+?.*?(;|(?=\s*{))/gi,inside:{punctuation:/[;:]/g}},url:/url\((["']?).*?\1\)/gi,selector:/[^\{\}\s][^\{\};]*(?=\s*\{)/g,property:/(\b|\B)[\w-]+(?=\s*:)/ig,string:/("|')(\\?.)*?\1/g,important:/\B!important\b/gi,punctuation:/[\{\};:]/g,"function":/[-a-z0-9]+(?=\()/ig};
...
Prism.languages.clike={comment:[{pattern:/(^|[^\\])\/\*[\w\W]*?\*\//g,lookbehind:!0},{pattern:/(^|[^\\:])\/\/.*?(\r?\n|$)/g,lookbehind:!0}],string:/("|')(\\?.)*?\1/g,"class-name":{pattern:/((?:(?:class|interface|extends|implements|trait|instanceof|new)\s+)|(?:catch\s+\())[a-z0-9_\.\\]+/ig,lookbehind:!0,inside:{punctuation:/(\.|\\)/}},keyword:/\b(if|else|while|do|for|return|in|instanceof|function|new|try|throw|catch|finally|null|break|continue)\b/g,"boolean":/\b(true|false)\b/g,"function":{pattern:/[a-z0-9_]+\(/ig,
    inside:{punctuation:/\(/}},number:/\b-?(0x[\dA-Fa-f]+|\d*\.?\d+([Ee]-?\d+)?)\b/g,operator:/[-+]{1,2}|!|<=?|>=?|={1,3}|&{1,2}|\|?\||\?|\*|\/|\~|\^|\%/g,ignore:/&(lt|gt|amp);/gi,punctuation:/[{}[\];(),.:]/g};
...
$(document).ready(function(){var d;d=$("#sidebar-wrapper.reference").length?"/help/themes/bootstrap/js/ref-toc.html":"/help/themes/bootstrap/js/user-toc.html";$("#sidebar-wrapper").load(d,function(c,d,f){$("a[href='"+window.location.pathname+"']").last().replaceWith('<div id="toc"></div>');var b=$("#toc");b.parents("li").removeClass("collapsed").addClass("active");$(".topictitle1").first().after(function(){var a;a=$("#next-button").length?'<span class="crumb rootcrumb">'+$(".topictitle1").text()+
'</span><a href="#" id="next-link">Next</a></div>':'<span class="crumb rootcrumb">'+$(".topictitle1").text()+"</span></div>";b.parent("li.active").parents("li.active").each(function(){a='<span class="crumb"><a href="'+$(this).find("a:first").attr("href")+'">'+$(this).find("a:first").text()+'</a></span><i class="glyphicon glyphicon-chevron-right"></i>'+a});return a='<div class="breadcrumbs"><span class="crumb"><a href="'+$("#guide-title").find("a").attr("href")+'">'+$("#guide-title").find("a").text()+
'</a></span><i class="glyphicon glyphicon-chevron-right"></i>'+a});var e=$(window).height()-300;$(function(){$(".sidebar-nav").slimScroll({width:"inherit",height:e})});b.hide().tocify({context:"#page-content-wrapper",scrollTo:100,selectors:"h1,h2,h3,h4,h5,h6",extendPage:!1}).show(400).promise().done(function(){var a=$("#tocify-header0").offset().top-400+"px";$(".sidebar-nav").slimScroll({scrollTo:a})});$("li.collapsible.collapsed a").append('<i class="glyphicon glyphicon-chevron-right"></i>');$("li.collapsible.active a:first").append('<i class="glyphicon glyphicon-chevron-down"></i>');
    $("#next-button, #next-link").click(function(a){if(b.next("ul").length)window.location.href=b.next("ul").first("li").find("a:first").attr("href");else if(b.parent("li.active").next("li").length)window.location.href=b.parent("li.active").next("li").find("a:first").attr("href");else{var c=!1;b.parents("li.active").each(function(){if($(this).next("li").length)return window.location.href=$(this).next("li").find("a:first").attr("href"),c=!0,!1});c||$("#next-button").attr("title","Last topic reached.")}});
    $("#search-form").find("input").keypress(function(a){13==a.which&&(window.location.href="/help/search.html?zoom_query="+encodeURIComponent($(this).val()))})});$("img").each(function(){var c=$(this).attr("src"),d=$(this).attr("id");$(this).wrap('<a href="'+c+'" data-lightbox="'+d+'"></a>')});$("#menu-toggle").click(function(c){c.preventDefault();$("#wrapper").toggleClass("toggled")})});var rtime=new Date(1,1,2E3,12,0,0),timeout=!1,delta=0;
$(window).resize(function(){rtime=new Date;!1===timeout&&(timeout=!0,setTimeout(resizeend,delta))});function resizeend(){new Date-rtime<delta?setTimeout(resizeend,delta):(timeout=!1,location.reload())};