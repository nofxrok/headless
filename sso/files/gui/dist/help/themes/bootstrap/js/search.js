...
document.write("<script language=\"JavaScript\" src=\"themes/bootstrap/js/zoom_index.js\" charset=\"" + Charset + "\"><\/script>");
document.write("<script language=\"JavaScript\" src=\"themes/bootstrap/js/zoom_pageinfo.js\" charset=\"" + Charset + "\"><\/script>");
...
{
	//update this with new search box
    document.writeln('<div id="search-form-full" class="inner-addon left-addon"><i class="glyphicon glyphicon-search"></i>');



    document.writeln('<input type="text" name="zoom_query" size="20" value="' + htmlspecialchars(query) + '" id="zoom_searchbox" class="zoom_searchbox search-form form-control" />');
...
{
	document.writeln("<input type=\"submit\" value=\"" + STR_FORM_SUBMIT_BUTTON + "\" class=\"zoom_button btn btn-primary\" />");
}
...
function ZoomShowCategories()
{
	if (UseCats) {
		...
		{
			document.write("<select class=\"btn btn-default dropdown-toggle\" name='zoom_cat[]'>");
			// 'all cats option
...
{
	document.writeln("<span class=\"zoom_match input-group\">");
	if (andq == 0) {
		document.writeln("<span class=\"input-group-addon\"><input type=\"radio\" name=\"zoom_and\" value=\"0\" checked=\"checked\" />" + STR_FORM_ANY_SEARCH_WORDS + "</span>" );
		document.writeln("<span class=\"input-group-addon\"><input type=\"radio\" name=\"zoom_and\" value=\"1\" />" + STR_FORM_ALL_SEARCH_WORDS + "</span>");
	} else {
...
function ZoomShowHeading()
{
	...
	document.write("<div class=\"searchheading\">" + STR_RESULTS_FOR + " " + queryForHTML + "</div>");
	if (UseCats) {
		if (cat[0] == -1)
		{
		...
		{
			document.writeln(' <div class="categories">' + STR_RESULTS_IN_CATEGORY + " ");
			for (catit = 0; catit < num_zoom_cats; catit++)
			{
				if (catit > 0)
					document.write(", ");
	...
	for (catit = 0; catit < NumCats; catit++)
	{
		if (CatCounter[catit] > 0)
		{
			...
			if (CatCounter[catit] != matches)
			{
				...
				{
					document.writeln("<div class=\"cat_summary\"><br /><span class=\"summary_title\"" + STR_CAT_SUMMARY + "</span><ul>");
