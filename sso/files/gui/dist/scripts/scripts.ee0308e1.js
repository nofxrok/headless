    ...
    .controller('MinionsCtrl', ['$scope', '$rootScope', 'SaltMinionService', '$timeout', '$state', '$stateParams', 'lodash', '$modal', 'growl', function($scope, $rootScope, SaltMinionService, $timeout, $state, $stateParams, lodash, $modal, growl) {
        ...
        $scope.selectMinion = function(index, minionId) {
            ...
            //Else remove it
                ...
                //Atleast one minion to be selected
                if($scope.selectedMinions.length === 1) {
                    $scope.checkedMinions[index] = true;
                    growl.error('Atleast one minion you have to select');
                } else {
                    //if removing item added last
                    if(newIndex+1 === $scope.selectedMinions.length) {
