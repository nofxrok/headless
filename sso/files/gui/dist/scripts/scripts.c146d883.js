 ...
 */
angular.module('sseFeApp')
    .controller('DashboardCtrl', ['$scope', 'SaltAuthService', 'SaltConfig', 'SaltMasterService', 'SaltMinionService', 'SaltJobService', '$window', '$state', 'lodash', '$modal', 'growl', 'WebSockService', '$stateParams', function($scope, SaltAuthService, SaltConfig, SaltMasterService, SaltMinionService, SaltJobService, $window, $state, lodash, $modal, growl, WebSockService, $stateParams) {
        ...
        $scope.openJobSchedular = function(selectdJob) {
            ...
            /**
             * Get Selected Minion List
             * to Execute a Job
             */
            $scope.get = function(){
                $scope.$broadcast ('getSelectedMinions');
                return  $scope.selectedMinions;
            };
            
            $scope.$on('sendSelectedMinions', function(e,data) {  
                $scope.selectedMinions = data;
            });
            $scope.get();
            // Job 
            /*jshint camelcase: false */
            ...
            $scope.job.end_after = '';
            $scope.job.minions = $scope.selectedMinions;
...
angular.module('sseFeApp')
    .controller('MinionsCtrl', ['$scope', '$rootScope', 'SaltMinionService', '$timeout', '$state', '$stateParams', 'lodash', '$modal', 'growl', function($scope, $rootScope, SaltMinionService, $timeout, $state, $stateParams, lodash, $modal, growl) {
        ...
        /**
         * Send Minion list to Dashboard.js for job scheduling.
         */
        
        $scope.$on('getSelectedMinions', function() {
            $scope.$emit('sendSelectedMinions', $scope.getSelectedMinionsList());
        });
        ...
        $scope.getQtMinions = function(qtPage) {
            ...
            //List Quick Targets
            SaltMinionService.getQuickTargetMinions(qtPage)
            .then(function(response) {
                if(response.data.data.count > 0) {
                    $scope.minions = response.data.data.results;
        ...
        $scope.getTargetMinions = function(tPage) {
            //Set page number if available
              ...
              SaltMinionService.getTargetMinions($stateParams.targetId, tPage)
              .then(function(response) {
                  if(response.data.data.count > 0) {
                      $scope.minions = response.data.data.results;
        ...
        $scope.constructGrainsData = function() {
            ...
            //Format the data
            if(typeof $scope.grainsKeySelectedOne !== 'undefined' && typeof $scope.grainsValueSelectedOne !== 'undefined') {
            ...
            if(typeof $scope.grainsKeySelectedTwo !== 'undefined' && typeof $scope.grainsValueSelectedTwo !== 'undefined') {
            ...
            if(typeof $scope.grainsKeySelectedThree !== 'undefined' && typeof $scope.grainsValueSelectedThree !== 'undefined') {
        ...
        $scope.avoidDuplicateSelection = function(which) {
            switch(which) {
                case 1:
                    if(typeof $scope.grainsKeySelectedOne !== 'undefined' && ($scope.grainsKeySelectedOne === $scope.grainsKeySelectedTwo || $scope.grainsKeySelectedOne === $scope.grainsKeySelectedThree)) {
                        growl.error('Option already selected');
                        $scope.grainsKeySelectedOne = '';
                    }
                    $scope.grainsValueSelectedOne = undefined;
                break;

                case 2:
                    if(typeof $scope.grainsKeySelectedTwo !== 'undefined' && ($scope.grainsKeySelectedTwo !== '' && $scope.grainsKeySelectedTwo === $scope.grainsKeySelectedOne || $scope.grainsKeySelectedTwo === $scope.grainsKeySelectedThree)) {
                        growl.error('Option already selected');
                        $scope.grainsKeySelectedTwo = '';
                    }
                    $scope.grainsValueSelectedTwo = undefined;
                break;

                case 3:
                    if(typeof $scope.grainsKeySelectedThree !== 'undefined' && ($scope.grainsKeySelectedThree !== '' && $scope.grainsKeySelectedThree === $scope.grainsKeySelectedOne || $scope.grainsKeySelectedThree === $scope.grainsKeySelectedTwo)) {
                        growl.error('Option already selected');
                        $scope.grainsKeySelectedThree = '';
                    }
                    $scope.grainsValueSelectedThree = undefined;
                break;
            }

            if($scope.grainsKeySelectedOne === undefined && $scope.grainsKeySelectedTwo === undefined && $scope.grainsKeySelectedThree === undefined && $scope.grainsValueSelectedOne === undefined && $scope.grainsValueSelectedTwo === undefined && $scope.grainsValueSelectedThree === undefined) {
                $scope.filterListLabel = 'No filter';
        ...
        /**
         * Navigate to job history by minions page with selected minions data
         */
        $scope.goToMinionsJobHistory = function() {
            if($scope.selectedMinions.length > 0) {
                $rootScope.selectedMinions = $scope.selectedMinions;
                $state.go('salt.dashboard.resources.minions-job-history');
            }
        };
        ...
        $scope.sendKeyAction = function() {
            var actionData = { minions: $scope.selectedMinions, action: $scope.keyAction };

            SaltMinionService.doKeyAction(actionData)
            /*jshint unused:false*/
            .then(function(response) {
                switch($scope.keyAction) {
                    case 'accept':
