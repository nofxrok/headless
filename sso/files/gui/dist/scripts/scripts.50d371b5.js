            ...
            if(typeof $scope.sortingParameter !== 'undefined') {
              ...
              .then(function(response) {
                  if(response.data.data.count > 0) {
                      ...
                      //Checking for previous selected minions
                      if($rootScope.currentPage === $scope.currentTPage){
                          if($rootScope.previousState === 'salt.dashboard.resources.target-reports' || $rootScope.previousState === 'salt.dashboard.resources.job-history' || $rootScope.previousState === 'salt.dashboard.resources.target-eventstream'){
                              /*jshint unused:false*/
                              $rootScope.selectedMinions.forEach(function(v, i){
                                  $scope.minions.forEach(function(val, ind){
                                      if(val.id === v){
...
//        };
        ...
        $scope.applyTextFilter = function(minionsPage) {
            ...
            } else if($state.current.name === 'salt.dashboard.resources.folder') {
