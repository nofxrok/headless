            ...
            .then(function(response) {
                if(response.data.data.count > 0) {
                    ...
                    if($state.current.name !== 'salt.dashboard.resources.minions.detail') {
                        $state.go('salt.dashboard.resources.minions.detail',{'minionId':$scope.minions[0].id, 'view':'list'});
            ...
            .then(function(response) {
                if(response.data.data.count > 0) {
                    ...
                    if($state.current.name !== 'salt.dashboard.resources.quicktargets.minions') {
                        $state.go('salt.dashboard.resources.quicktargets.minions',{'minionId':$scope.minions[0].id, 'view':'list'});
                        $scope.selectMinion(0, $scope.minions[0].id);
                    }
              ...
              .then(function(response) {
                  if(response.data.data.count > 0) {
                      ...
                      if($state.current.name !== 'salt.dashboard.resources.targets.minions') {
                          $state.go('salt.dashboard.resources.targets.minions',{'minionId':$scope.minions[0].id, 'view':'list'});
                          $scope.selectMinion(0, $scope.minions[0].id);
                      }
                      //Show pagination if required
            ...
            link: function ( scope, element, attrs ) {
                ...
                case 'publicTargetTree':
                    template =
                        ...
                        '<span class="child-node-tree" ng-hide = "hover" ui-sref="salt.dashboard.resources.targets({ targetId: target.id })" ng-class="{ selected: state.includes(\'salt.dashboard.resources.job-history\', { targetId: target.id }) || state.includes(\'salt.dashboard.resources.target-reports\', { targetId: target.id })  || state.includes(\'salt.dashboard.resources.targets\', { targetId: target.id }) || state.includes(\'salt.dashboard.resources.target-beacons\', { targetId: target.id }) }">{{ target.target_name }}</span>' +
