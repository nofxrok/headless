...
angular.module('sseFeApp')
    .controller('DashboardCtrl', ['$scope', '$rootScope', 'SaltAuthService', 'SaltConfig', 'SaltMasterService', 'SaltMinionService', 'SaltJobService', '$window', '$state', 'lodash', '$modal', 'growl', 'WebSockService', '$stateParams', '$modalStack', function($scope, $rootScope, SaltAuthService, SaltConfig, SaltMasterService, SaltMinionService, SaltJobService, $window, $state, lodash, $modal, growl, WebSockService, $stateParams, $modalStack) {
        ...
        $scope.disablePast = function($view, $dates) {
            /*jshint -W030 */
            var today           = new Date(),
                todayMidnight   = new Date(today.getFullYear(), today.getMonth(), today.getDate(), 0,0,0),
                todayMonth      = today.getMonth(),
                todayYear       = today.getFullYear(),
                todayHour       = today.getHours(),
                todayMinute     = today.getMinutes(),
                timezoneOffsetHours     = Math.floor(Math.abs(today.getTimezoneOffset()) / 60),
                timezoneOffsetMinutes   = Math.abs(today.getTimezoneOffset()) % 60,
                date = 0,
                dateMidnight = 0,
                dateMonth = 0,
                dateHour = 0,
            ...
            if($view === 'day') {
                for(i = 0; i < $dates.length; i++) {
                    /*jshint -W030 */
                    date            = new Date($dates[i].dateValue),
                    dateMidnight    = new Date(date.getFullYear(), date.getMonth(), date.getDate(), 0,0,0);
            ...
            //Disable hours before current hour
                ...
                for(i = 0; i < $dates.length; i++) {
                    /*jshint -W030 */
                    date        = new Date($dates[i].dateValue),
                    dateHour    = date.getHours() - timezoneOffsetHours,
                    adjusted    = new Date(date.getFullYear(), date.getMonth(), date.getDate(), dateHour),
                    dateMidnight = new Date(adjusted.getFullYear(), adjusted.getMonth(), adjusted.getDate(), 0, 0, 0);

                    //Adjust hours on a scale of 24
                    //24 = 0, hence anything above 23 results in a negative value when offset is subtracted
                    if(dateHour < 0) {
                        dateHour = 24 + dateHour;
                    }

                    //Disable all hours before current hour
                    if(Number(dateMidnight) === Number(todayMidnight)) {
                        if(dateHour < todayHour) {
                            $dates[i].selectable = false;
                        }
                    } else if(Number(dateMidnight) < Number(todayMidnight)) {
                        $dates[i].selectable = false;
                    }
                }
            } else {
                for(i = 0; i < $dates.length; i++) {
                    ...
                    difference  = minutes - timezoneOffsetMinutes;
                    ...
                    //If the difference is negative, lose an hour
                    if(difference < 0) {
                        dateHour = dateHour - 1;
                    ...
                    //Disable all hours before current hour
                    if(Number(dateMidnight) === Number(todayMidnight)) {
                        //Disable all minutes for the current hour before current minute
                        if(dateHour === todayHour) {
                            //If the minute 
                            if(difference < todayMinute) {
                                $dates[i].selectable = false;
                            }
                        } else if(dateHour < todayHour) {
                            $dates[i].selectable = false;
                        }
                    } else if(Number(dateMidnight) < Number(todayMidnight)) {
                        $dates[i].selectable = false;
...
//                    if($state.current.name !== 'salt.dashboard.resources.minions.detail') {
            ...
            link: function ( scope, element, attrs ) {
                ...
                case 'publicTargetTree':
                    template =
                        ...
                        '<span class="child-node-tree" ng-hide = "hover" ui-sref="salt.dashboard.resources.targets({ targetId: target.id, view: \'list\'})" ng-class="{ selected: state.includes(\'salt.dashboard.resources.job-history\', { targetId: target.id }) || state.includes(\'salt.dashboard.resources.target-reports\', { targetId: target.id })  || state.includes(\'salt.dashboard.resources.targets\', { targetId: target.id }) || state.includes(\'salt.dashboard.resources.target-beacons\', { targetId: target.id }) }">{{ target.target_name }}</span>' +
